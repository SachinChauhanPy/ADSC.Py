...
export function Header() {
  ...
  return (
    <header className="sticky top-0 z-50 bg-white/95 backdrop-blur-sm border-b-2 border-zinc-900 shadow-sm">
        ...
        <div className="flex items-center justify-between h-20">
          ...
          <Link to="/" className="flex items-center gap-3 group">
            ...
            <div className="flex flex-col">
              <div className="flex items-center gap-1.5">
                <span className="font-pixel text-xl tracking-tight text-zinc-900 font-bold">
                  {baseName}
                  {endsWithPy && (
                    <>
              ...
              return (
                <Link
                  ...
                  className={`px-3 py-2 text-sm font-semibold transition-all ${
                    active
                      ? 'bg-zinc-900 text-white border-2 border-zinc-900 shadow-[2px_2px_0px_#4285F4]'
                      : 'text-zinc-700 hover:text-zinc-900 hover:bg-zinc-100 border-2 border-transparent'
                  }`}
                >
                  {item.label}
                </Link>
          ...
          <div className="hidden sm:flex items-center gap-3">
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="pixel-btn text-xs px-3 py-2 flex items-center gap-1.5 text-zinc-800"
            >
              <span>Discord</span>
          ...
          <div className="lg:hidden flex items-center">
            <button
              ...
              className="pixel-btn p-2 text-zinc-900"
      ...
      {/* Mobile Drawer */}
      {mobileMenuOpen && (
        <div className="lg:hidden border-t-2 border-zinc-900 bg-white p-4 space-y-2">
          {navItems.map((item) => (
            <Link
              key={item.href}
              to={item.href}
              ...
              className={`block px-4 py-2.5 font-semibold text-base border-2 ${
                isActive(item.href)
                  ? 'bg-zinc-900 text-white border-zinc-900 shadow-[3px_3px_0px_#4285F4]'
                  : 'border-zinc-900 text-zinc-800 hover:bg-zinc-50'
              }`}
            >
              {item.label}
            </Link>
