...
import { ArrowRight, CheckCircle2, Code2, Sparkles, Layers } from 'lucide-react';
  ...
  return (
    <Shell>
      <div className="space-y-12">
        ...
        {/* Quick Nav Pills */}
        <div className="flex flex-wrap items-center justify-center gap-2 py-4 border-y-2 border-zinc-900 bg-zinc-50/50 p-4">
          <span className="font-pixel text-xs text-zinc-500 uppercase mr-2">JUMP TO DOMAIN:</span>
          {domains.map((domain) => (
            <a
              key={domain.id}
              href={`#${domain.id}`}
              className="pixel-btn text-xs px-3 py-1.5 font-bold text-zinc-800 bg-white hover:bg-[#FFD43B]"
            >
              {domain.title}
            </a>
          ))}
        ...
        <div className="space-y-16">
          {domains.map((domain, index) => (
            <div
              key={domain.id}
              id={domain.id}
              className="pixel-card-static p-6 sm:p-10 bg-white border-2 border-zinc-900 scroll-mt-28 space-y-8 relative"
            >
              {/* Header */}
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b-2 border-zinc-100 pb-6">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <h2 className="text-2xl sm:text-3xl font-extrabold text-zinc-900 mt-1 hover:text-[#4285F4] transition-colors">
              ...
              {/* Project Blueprints */}
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4 border-t-2 border-zinc-100">
